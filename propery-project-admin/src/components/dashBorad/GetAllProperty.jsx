  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        let response;
        if (sortOption === "all") {
          response = await axios.get("http://localhost:4000/api/property/properties");
        } else {
          response = await axios.get(`http://localhost:4000/api/property/properties?sort=${sortOption}`);
        }
  ...
  return (
    <div className="bg-slate-100">
      ...
      <div className="flex justify-center mt-4 absolute right-16">
        <label htmlFor="sortSelect" className="mr-2  text-black font-medium text-lg items-center px-2 mt-1">
          Sort by:
        </label>
        <select
          id="sortSelect"
        ...
        >
           <option value="all" className="hover:bg-inherit">All</option>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-20 ">
        {currentProperties.map((properties, index) => (
          <div className="flex  m-4 rounded-lg shadow-md bg-white bg-opacity-25" key={index}>
            <div className="">
              <img
            ...
            <div className="mx-4 items-center ">
              ...
              <div className="flex  ">
               <Link to={`/update/${properties._id}`}> <button className=" mx-2 text-2xl ">
                  <span className="material-symbols-outlined ml-5 text-3xl">edit_note</span>
                </button> </Link>
                <div className=" flex-col">
                <DeleteProperty propertyDetails={properties} propertyId={properties._id} onDelete={handleDelete} />
        ...
        <ul className="flex">
          {Array.from({ length: Math.ceil(properties.length / propertiesPerPage) }).map((_, index) => (
            <li key={index}>
              <button
                className={`px-3 py-1 ${
                  currentPage === index + 1 ? "bg-red-400 text-white" : "bg-red-500"
                } mr-1 rounded-full`}
      ...
      <button className=" p-2 bg-red-500 text-white  rounded-full font-extrabold absolute right-0 bottom-0 sm:absolute sm:right-0 sm:bottom-0 "   onClick={()=>navigate("/dashboard")}>  <span className="material-symbols-outlined">
arrow_back
</span></button>
    </div>
  );
