...
function DashBoardHomePage() {
  ...
  useEffect(()=>{
    const leads=async()=>{
      try {
        const response=await axios.get("http://localhost:4000/api/otp/total-user");
        setLead(response.data)
        console.log(response.data);
        const allProperties=await axios.get("http://localhost:4000/api/property/property-count");
