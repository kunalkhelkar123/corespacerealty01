...
function PropertySubmission() {
  ...
  const handleAmenityChange = (amenityName, isChecked) => {
    if (isChecked) {
      ...
      setFormData({
        ...formData,
        amenities: formData.amenities.filter(amenity => amenity !== amenityName)
      });
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4000/api/property/propertyDetails",
        formData
      );
      ...
      // Add any further logic here (e.g., show success message)
    } catch (error) {
          ...
          <form className=" w-full p-8" onSubmit={handleSubmit}>
            <div className=" bg-white p-5 w-full">
              ...
              <div>
                ...
                <input
                  className="placeholder:italic h-40 bg-white w-full border border-slate-300 rounded-md py-2 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500  sm:text-sm"
                  placeholder=""
                  type="textarea"
